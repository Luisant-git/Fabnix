...
const Header = () => {
  ...
  const [desktopSearchRecommendations, setDesktopSearchRecommendations] =
    useState([]);
          ...
          {categories.map((category) => (
            <div key={category.id} className="nav-item">
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  navigate(generateCategoryUrl(category.name, category.id));
                }}
              >
                {category.name}
              </a>
                  ...
                  {category.subCategories.map((subcat) => (
                    <a
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        navigate(
                          generateSubcategoryUrl(subcat.name, subcat.id)
                        );
                      }}
                    >
      ...
      <div className="header-right">
        <form className="search-container" onSubmit={handleSearch}>
          ...
          {searchQuery && (
            <button
              type="button"
              className="clear-desktop-search-btn"
              onClick={() => {
                setSearchQuery("");
                setDesktopSearchRecommendations([]);
              }}
          ...
          {desktopSearchRecommendations.length > 0 && (
            <div className="desktop-search-recommendations">
              {desktopSearchRecommendations.map((product) => (
                <div
                  key={product.id}
                  className="desktop-search-recommendation"
                  onClick={() => {
                ...
                >
                  ...
                  <div className="desktop-search-recommendation-info">
                    <p className="desktop-search-recommendation-name">
                      {product.name}
                    </p>
                    <p className="desktop-search-recommendation-price">
                      ₹{product.price}
                    </p>
                  </div>
                </div>
        ...
        <div className="header-actions">
          ...
          <div
            className="icon-wrapper mobile-search-icon"
            style={{ display: "none" }}
          ...
          >
            ...
            {compare.length > 0 && (
              <span className="badge">{compare.length}</span>
            )}
          </div>
          ...
          <div className={`mobile-nav ${isMobileMenuOpen ? "open" : ""}`}>
            ...
            <nav>
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  handleMobileLinkClick("/new-arrivals");
                }}
              >
                New Arrivals
              </a>
              ...
              {categories.map((category) => (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    handleMobileLinkClick(
                      generateCategoryUrl(category.name, category.id)
                    );
                  }}
                >
                  {category.name}
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  handleMobileLinkClick(user ? "/profile" : "/login");
                }}
              >
                {user ? "Profile" : "Login"}
              </a>
              ...
              {searchRecommendations.map((product) => (
                ...
                >
                  ...
                  <div className="search-recommendation-info">
                    ...
                    <p className="search-recommendation-price">
                      ₹{product.price}
                    </p>
                  </div>
                </div>
